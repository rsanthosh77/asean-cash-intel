...
st.markdown("""
<style>
@import url('https://fonts.googleapis.com/css2?family=Playfair+Display:wght@400;700;900&family=/* ── Reset ── */
...
[data-baseweb="popover"] { background: var(--sf2) !important; border: 1px solid var(--bd2) !important; [data-baseweb="menu"] li { color: var(--dim) !important; font-family: 'IBM Plex Mono',monospace [data-baseweb="menu"] li:hover { background: var(--sf3) !important; }
.stMultiSelect input { color: var(--dim) !important; font-size: 10px !important; }
div[data-baseweb="select"] span { color: var(--mu) !important; font-family: 'IBM Plex Mono',monospace /* Style native Streamlit widget labels in our custom sidebar column */
...
/* ── Metric cards ── */
.mcard { background: var(--sf); border: 1px solid var(--bd); border-radius: 8px; padding: 13px .mcard::after { content:''; position:absolute; top:0;left:0;right:0; height:2px; }
...
.mlbl { font-family:'IBM Plex Mono',monospace; font-size:9px; letter-spacing:.12em; color:var(--.mval { font-family:'Playfair Display',serif; font-size:28px; font-weight:700; color:var(--tx); .msub { font-family:'IBM Plex Mono',monospace; font-size:9px; color:var(--mu); -webkit-font-smoothing:.mhint { font-family:'IBM Plex Mono',monospace; font-size:8px; letter-spacing:.08em; color:var(--/* ── Signal cards ── */
.signal-card { background:var(--sf); border:1px solid var(--bd); border-radius:8px; padding:13px .signal-card:hover { border-color:var(--bd2); transform:translateX(2px); }
.sc-row { display:flex; align-items:flex-start; gap:11px; margin-bottom:7px; }
.sc-score { width:28px; height:28px; border-radius:5px; display:flex; align-items:center; justify-.s5{background:rgba(0,194,168,.12);color:var(--teal);}
...
.tag{font-family:'IBM Plex Mono',monospace;font-size:9px;letter-spacing:.04em;padding:2px 6px;.tag-entity{background:rgba(0,194,168,.1);color:var(--teal);border:1px solid rgba(0,194,168,..tag-geo{background:rgba(37,99,235,.1);color:#93c5fd;border:1px solid rgba(37,99,235,.18);}
.tag-geo-wide{background:rgba(37,99,235,.16);color:#bfdbfe;border:1px solid rgba(37,99,235,.3);}
.tag-geo-global{background:rgba(100,116,139,.1);color:#94a3b8;border:1px solid rgba(100,116,139,..tag-impact{background:rgba(0,194,168,.06);color:#5eead4;border:1px solid rgba(0,194,168,.12);.tag-product{background:rgba(100,116,139,.08);color:#8a9ab2;border:1px solid rgba(100,116,139,..tag-signal{background:rgba(245,158,11,.08);color:var(--ora);border:1px solid rgba(245,158,11,..tag-pdf{background:rgba(139,92,246,.1);color:#c4b5fd;border:1px solid rgba(139,92,246,.2);}
.tag-consultant{background:rgba(20,184,166,.1);color:#2dd4bf;border:1px solid rgba(20,184,166,..tag-regulatory{background:rgba(239,68,68,.1);color:#fca5a5;border:1px solid rgba(239,68,68,..sc-hl{font-size:13px;font-weight:500;color:var(--tx);line-height:1.45;margin-bottom:5px;font-.sc-sw{font-size:11px;color:var(--dim);line-height:1.55;font-family:'IBM Plex Sans',sans-serif;}
.sc-sw strong{color:var(--teal);font-weight:500;}
.sc-ft{display:flex;align-items:center;justify-content:space-between;margin-top:8px;padding-top:.sc-dt{font-family:'IBM Plex Mono',monospace;font-size:9px;color:var(--mu);-webkit-font-smoothing:.sc-src-lnk{font-family:'IBM Plex Mono',monospace;font-size:9px;color:var(--blue);text-decoration:/* ── Bar chart ── */
.bar-chart{display:flex;flex-direction:column;gap:6px;}
.bar-row{display:flex;align-items:center;gap:7px;}
.bar-lbl{font-family:'IBM Plex Mono',monospace;font-size:9.5px;color:var(--dim);width:80px;flex-.bar-bg{flex:1;height:4px;background:var(--sf3);border-radius:3px;overflow:hidden;}
.bar-fill{height:100%;border-radius:3px;background:linear-gradient(90deg,var(--teal),var(--blue));}
.bar-n{font-family:'IBM Plex Mono',monospace;font-size:9.5px;color:var(--mu);width:16px;text-/* ── Misc ── */
.sec-hdr{font-family:'IBM Plex Mono',monospace;font-size:9px;letter-spacing:.14em;color:var(--.geo-note{background:rgba(37,99,235,.05);border:1px solid rgba(37,99,235,.15);border-radius:5px;
.drill-panel{background:var(--sf);border:1px solid rgba(0,194,168,.28);border-radius:8px;padding:.drill-title{font-family:'IBM Plex Mono',monospace;font-size:8px;letter-spacing:.14em;text-transform:.page-info{text-align:center;font-family:'IBM Plex Mono',monospace;font-size:8.5px;color:var(--.chat-user{background:var(--sf2);border:1px solid var(--bd2);border-radius:6px;padding:10px 14px;.chat-bot{background:var(--sf);border:1px solid var(--bd);border-radius:2px 8px 8px 8px;padding:.chat-bot strong{color:var(--teal);}
.chat-err{background:rgba(239,68,68,.05);border:1px solid rgba(239,68,68,.2);border-radius:6px;/* Tabs */
.stTabs [data-baseweb="tab-list"]{background:transparent !important;border-bottom:1px solid var(--.stTabs [data-baseweb="tab"]{background:transparent !important;color:var(--mu) !important;font-.stTabs [aria-selected="true"]{color:var(--teal) !important;border-bottom-color:var(--teal) !/* Buttons */
.stButton button{font-family:'IBM Plex Mono',monospace !important;font-size:10px !important;letter-.stButton button[kind="primary"]{background:var(--teal) !important;color:#000 !important;border:.stButton button[kind="primary"]:hover{background:#00d9bb !important;transform:translateY(-1px) .stButton button[kind="secondary"]{
...
.stButton button[kind="secondary"]:hover{border-color:var(--teal) !important;color:var(--teal) /* Expander */
div[data-testid="stExpander"]{background:var(--sf) !important;border:1px solid var(--bd) !important;div[data-testid="stExpander"] summary{font-family:'IBM Plex Mono',monospace !important;font-size:/* Text input */
.stTextInput input{background:var(--sf2) !important;border:1px solid var(--bd2) !important;color:.stTextInput input:focus{border-color:rgba(0,194,168,.45) !important;box-shadow:0 0 0 2px rgba(/* Typography */
.stMarkdown h1,.stMarkdown h2,.stMarkdown h3{font-family:'Playfair Display',serif !important;p,li,.stMarkdown p{color:#cbd5e1 !important;font-family:'IBM Plex Sans',sans-serif !important;}
...
if "regulatory" in src or s.get("signal_type","").lower() == "regulatory update": return if "bank-product" in src or t in ("scrape-static", "scrape"): return "bank-product"
return "news"
...
with c2:
st.markdown(f'<div class="page-info">Page {p} of {pages}</div>', unsafe_allow_html=with c3:
...
else f"Fresh · {newest.strftime('%d %b %Y') if newest else 'today'} · {len(signals)} st.markdown(f"""
...
"◈ Overview", " Signals", "◎ Ask Intelligence", " Intelligence Library", "▤ ])
# ═══════════════════
# TAB 1 — OVERVIEW
# ═══════════════════
with tab1:
...
st.markdown(f'<div class="geo-note" style="margin-top:10px"> {" · ".join(notes)}</unsafe_allow_html=True)
...
with lc:
st.markdown('<div class="sec-hdr"> Top Signals This Period</div>', unsafe_allow_if fs:
...
if top_e: st.markdown(bar_html(top_e, max(c for _,c in top_e)), unsafe_allow_html=st.markdown("<div style='height:14px'></div>", unsafe_allow_html=True)
...
if geo_filter:
gn = ["ASEAN-Wide (all markets + ASEAN impact)" if g=="ASEAN-Wide" else f"{g} only" st.markdown(f'<div class="geo-note"> {" · ".join(gn)}</div>', unsafe_allow_html=# ── View A: Market Signals ──
...
unsafe_allow_html=True)
disp_dated = dated_fs
if sort_by == "Date": disp_dated = sorted(dated_fs, key=lambda x:x.get("date",""), elif sort_by == "Entity": disp_dated = sorted(dated_fs, key=lambda x:x.get("entity",""))
...
<div style="font-family:'IBM Plex Mono',monospace;font-size:9px;letter-spacing:.NO DATED SIGNALS MATCH YOUR FILTERS</div></div>""", unsafe_allow_html=else:
...
Bank product pages, consultant PDFs, research reports · no publish date · not <div style="font-family:'IBM Plex Mono',monospace;font-size:7.5px;
color:#475569;padding-bottom:10px">
For the full library view, see the Intelligence Library tab →</div>''',
unsafe_allow_html=True)
disp_undated = undated_fs
if sort_by == "Entity": disp_undated = sorted(undated_fs, key=lambda x:x.get("elif sort_by == "Geography": disp_undated = sorted(undated_fs, key=lambda x:x.get("if not disp_undated:
st.markdown('''<div style="text-align:center;padding:40px 20px;color:#475569">
<div style="font-size:22px;margin-bottom:8px">◎</div>
<div style="font-family:'IBM Plex Mono',monospace;font-size:9px;letter-spacing:.NO REFERENCE ITEMS MATCH YOUR FILTERS</div></div>''', unsafe_allow_html=else:
...
{len(dated_chat)} market signals (last 45 days) + {len(undated_chat)} competitive </div>""", unsafe_allow_html=True)
...
placeholder="Ask about ASEAN cash management intelligence… (press Enter or label_visibility="collapsed", key="chat_input")
with ci2:
...
g = top_geos[0]
if g == "ASEAN-Wide":
cands.append("What are the biggest cash management developments across ASEAN else:
...
cands.append("What are the most important market trends in ASEAN cash management
...
if has_innov:
cands.append("What are the latest real-time payment and innovation signals in else:
...
if msg.get("role")=="user" and msg.get("answer") is None and msg.get("error") is with st.spinner("Searching signals…"):
ctx="".join(
f"Entity:{s.get('entity','')}\nGeo:{s.get('geography','')}\n"
f"ASEAN Impact:{s.get('asean_impact',True)}\nProduct:{s.get('product_f"Type:{s.get('signal_type','')}\nDate:{s.get('date','')[:10]}\n"
...
"You are a senior transaction banking analyst covering ASEAN "Answer using the intelligence signals below. Two types are "1. DATED signals (last 45 days) — recent news, regulatory "2. UNDATED signals — evergreen reference: bank product pages, "Signals marked ASEAN Impact: True are relevant even if non-"Be direct, cite entities, geographies, and dates where available.\"Weight consultant/PDF signals higher. Flag undated signals
"End with: 'Implication for Transaction Banks:' on a new "If insufficient signals, say so clearly.\n\n"
...
st.session_state.chat_history[i].update({"answer":answer,"sources":rel,"error":st.rerun()
for msg in st.session_state.chat_history:
if msg.get("role") != "user": continue
st.markdown(f'<div class="chat-user">{msg["content"]}</div>', unsafe_allow_html=True)
if msg.get("error"):
st.markdown(f'<div class="chat-err">⚠ {msg["error"]}</div>', unsafe_allow_html=elif msg.get("answer"):
st.markdown(f'<div class="chat-bot">{msg["answer"]}</div>', unsafe_allow_html=srcs = msg.get("sources") or []
if srcs:
with st.expander(" Signals referenced", expanded=False):
for r in srcs:
icon = " " if "pdf" in src_tag(r) else " "
g = r.get("geography","")
ai_n = " ⟶ ASEAN" if g not in ASEAN_ALL and r.get("asean_impact") url = r.get("url","")
ul = f" · [source]({url})" if url and not url.startswith("local://") st.caption(f"{icon} [{r.get('date','')[:10]}] **{r.get('entity','')}** f"({g}{ai_n}) — {r.get('key_signal','')[:80]}{ul}")
...
or when building competitive briefs. Also included in Ask Intelligence answers.</</div>""", unsafe_allow_html=True)
...
url = s.get("url","")
link = f'<a class="sc-src-lnk" href="{url}" target="_blank">&#8599; View</a>' entity = s.get("entity","") or ""
...
padding:2px 6px;border-radius:2px;margin-right:5px">{entity}</<span style="font-family:'IBM Plex Mono',monospace;font-size:8px;
...
url = s.get("url","")
link = f'<a class="sc-src-lnk" href="{url}" target="_blank">&#8599; View</a>' entity = s.get("entity","") or (s.get("source","") or "")[:40]
geo = s.get("geography","") or ""
sig = (s.get("key_signal","") or "")[:120]
sw = (s.get("so_what","") or "")[:120]
st.markdown(f'''<div style="background:#131a2e;border:1px solid #253348;
...
st.info("No research reports found. Add PDF URLs to sources.py or drop PDFs in documents/
# ═══════════════════════
# TAB 5 — WEEKLY DIGEST
# ═══════════════════════
with tab5:
...
<div style="font-family:'IBM Plex Sans',sans-serif;font-size:10.5px;color:#475569">
Auto-generated from the last 45 days · Includes consultant reports and PDFs</""", unsafe_allow_html=True)
dfiles = sorted(glob.glob("digest_*.md"), reverse=True)
if dfiles:
try:
dd = datetime.strptime(dfiles[0].replace("digest_","").replace(".md",""),"%dold = (datetime.now()-dd).days
except: dd=None; dold=0
with d2:
if dold>7:
st.markdown(f'<div style="background:rgba(239,68,68,.07);border:1px solid f'rgba(239,68,68,.2);border-radius:5px;padding:7px 11px;'
f'font-family:IBM Plex Mono,monospace;font-size:8.5px;'
f'color:#ef4444;margin-top:4px">⚠ {dold}d old</div>', unsafe_allow_html=sf = (st.selectbox("dig_sel", options=dfiles,
...
"research":"#8b5cf6","innovation":"#f59e0b","transaction":"#00c2a8","watch":"#for sec in dt.split("###")[1:]:
